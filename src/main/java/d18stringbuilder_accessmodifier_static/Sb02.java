...
public class Sb02 {
    public static void main(String[] args) {
        ...
        System.out.println(sb1.insert(2,"34"));

        //6-compareTo() metodunun görevi, iki StringBuilder Objesini karşılaştırmaktır.int değeri döndürür.Sozluk gibidir.
       ...
       /*Tek iş parçacığı, aynı anda sadece bir görevi işleyebilirken,
       birden fazla iş parçacığı aynı anda farklı görevleri işleyebilir ve
	...
	Kaynak Kontrolü: Tek iş parçacığı, belirli kaynakları (örneğin, dosya erişimi) kontrol altında tutmayı daha
	kolaylaştırır. Birden fazla iş parçacığı kullanmak, kaynakların paylaşılması ve senkronizasyon gerektirebilir,
	bu da daha karmaşık hale gelebilir.

	Performans Optimizasyonu: Bazı görevler tek iş parçacığı ile daha iyi çalışabilir. Özellikle,
	işlemci yoğun görevlerde tek iş parçacığı daha basit bir yaklaşım sunabilir. Ayrıca, belirli işlerin sıralı olarak
	yapılması gerektiğinde tek iş parçacığı kullanmak mantıklı olabilir.

	Platform Bağımsızlık: Tek iş parçacığı, farklı işletim sistemlerinde ve platformlarda daha tutarlı
	bir davranış sağlar. Birden fazla iş parçacığı kullanırken platforma bağımlı sorunlar ve davranış farklılıkları
	 ortaya çıkabilir.
